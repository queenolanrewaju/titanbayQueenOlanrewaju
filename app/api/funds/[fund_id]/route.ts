...
export async function GET(request: Request, { params }: { params: Promise<{ fund_id: string }> }) {
        ...
        const result = await pool.query('SELECT * FROM funds WHERE id = $1', [fund_id]); //$1 place holder, increments to $2, $3 for additional placeholders
...
//update a fund
export async function PUT(request:Request, { params }: { params: Promise<{ fund_id: string }> }) {
    try{
        const {fund_id} = await params;
        const { name, vintage_year, target_size_usd, status } = await request.json();
        const result = await pool.query(
        'UPDATE funds SET name = $1, vintage_year = $2,  target_size_usd = $3, status = $4 WHERE id = $5 RETURNING *', [name, vintage_year, target_size_usd, status, fund_id]
        );
